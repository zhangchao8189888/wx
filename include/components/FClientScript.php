<?php
/**
 * JClientScript manages JavaScript and CSS stylesheets for views.
 *
 * @author jinglin
 */
class FClientScript extends CApplicationComponent
{
	/**
	 ...
	 * will be inserted in the page. This can be one of the JClientScript::POS_* constants.
	 * Defaults to JClientScript::POS_HEAD.
	 * @since 1.1.3
	 */
	 ...
	 * rendering content. JClientScript thus gets a chance to insert script tags
	...
	public function renderCoreScripts()
	{
		...
		foreach ($this->coreScripts as $name => $package) {
			...
			if (isset($package['baseUrl'])) {
				...
				if ($baseUrl === '' || ($baseUrl[0] !== '/' && (strncasecmp($baseUrl, 'http', 4) !== 0))) {
	...
	{
		if (!isset($this->scriptFiles[self::POS_END]) 
				&& !isset($this->scripts[self::POS_END]) 
	 ...
	 * @return JClientScript the JClientScript object itself (to support method chaining, available since version 1.1.5).
	 * @since 1.1.7
	 ...
	 * <ul>
	 * <li>JClientScript::POS_HEAD : the script is inserted in the head section right before the title element.</li>
	 * <li>JClientScript::POS_BEGIN : the script is inserted at the beginning of the body section.</li>
	 * <li>JClientScript::POS_END : the script is inserted at the end of the body section.</li>
	 * </ul>
	 * @return JClientScript the JClientScript object itself (to support method chaining, available since version 1.1.5).
	 */
	 ...
	 * <li>JClientScript::POS_BEGIN : the script is inserted at the beginning of the body section.</li>
	 * <li>JClientScript::POS_END : the script is inserted at the end of the body section.</li>
	 * <li>JClientScript::POS_LOAD : the script is inserted in the window.onload() function.</li>
	 * <li>JClientScript::POS_READY : the script is inserted in the jQuery's ready function.</li>
	 * </ul>
	 * @return JClientScript the JClientScript object itself (to support method chaining, available since version 1.1.5).
	 */
	...
	public function registerMetaTag($content, $name = null, $httpEquiv = null, $options = array())
	{
		...
		if (!empty($name)) {
			if (!isset($this->metaTags[$name]) 
					|| isset($this->metaTags[$name]) && @$options['replace']) {
				$this->metaTags[$name] = $options;
			}
		} else {
