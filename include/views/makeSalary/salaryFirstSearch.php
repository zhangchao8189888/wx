...
<!--                                <td><div><a href="javascript:void();" class="updateStatus" data-status="--><?php //echo $row->salary_status?><!--" data-id="--><?php //echo $row->id;?><!--">--><?php //echo FConfig::item('config.examine_status.'.$row->salary_status);?><!--</a></div></td>-->
                            ...
                            <?php if ($row->salary_status < 1) {?><a title="继续添加" href="<?php echo $this->createUrl('toAddSalaryPage', array('id'=>$row->id));?>" style="cursor:pointer" class="theme-color">继续添加</a><?php } ?>
                                    <?php if ($row->salary_status < 1 || $row->salary_status == 3) {?><a title="删除" data-id="<?php echo $row->id;?>" data-type = "first" style="cursor:pointer" class="rowDel theme-color">删除</a>
                                <?php }
