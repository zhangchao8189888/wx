 ...
 *
 * The followings are the available columns in table '{{auth_group}}':
 * @property integer $id
...
class AuthGroup extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('rules', 'length', 'max'=>80),
			array('describe', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, status, rules, describe', 'safe', 'on'=>'search'),
		);
	}
